def run(files_dir, data_dir):
    ...
    stop_words = create_set_from_file(data_dir + '/stopwords.txt')
    composers = create_set_from_file(data_dir + '/composers.txt')
    musicians = create_set_from_file(data_dir + '/artists.txt') 
    soundtrack_composers = create_set_from_file(data_dir + '/soundtrack-composers.txt')
    genres = create_set_from_file(data_dir + '/genres.txt')
    videogames = create_set_from_file(data_dir + '/video-game-titles.txt')
    instruments = create_set_from_file(data_dir + '/instruments.txt')

    bb_songs = []
    with open(data_dir + '/billboard-top-100-1950-2015.txt') as f:
        bb_songs = [line.rstrip().lower().split(',') for line in f]

    song_titles = []
    with open(data_dir + '/top-10k-songs-with-artists.txt') as f:
